...
import { ThumbsUp, MessageCircle, Share2, MoreHorizontal, Globe, User2, Store, Users, PlaySquare, Image, Video, Smile } from 'lucide-react';
...
const SocialFeed = () => {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      if (error) {
        console.warn("Relationship logic failed, using direct fallback.");
  ...
  // Social Media Share Handler (Native)
    ...
    <div className="flex flex-col min-h-screen bg-[#F0F2F5] dark:bg-[#18191A] text-foreground transition-colors duration-300">
      ...
      <div className="flex justify-between w-full h-[calc(100vh-56px)] mt-[56px] overflow-hidden">
        ...
        <main className="flex-1 max-w-[680px] mx-auto overflow-y-auto pt-4 md:px-4 custom-scrollbar">
          ...
          {/* STORIES SECTION - No Create Button */}
              ...
              <div key={post.id} className="bg-card border border-border/50 rounded-xl mb-4 shadow-sm mx-2 overflow-hidden animate-in fade-in">
                ...
                <div className="px-4 py-2.5 flex justify-between text-muted-foreground text-[13px] border-b border-border/40">
                  ...
                  <div className="flex gap-3">
                    <span className="hover:underline cursor-pointer">14 comments</span>
                ...
                <div className="px-2 py-1 flex justify-between">
                  <ActionBtn icon={<ThumbsUp size={20} />} label="Like" />
                  <button 
                  ...
                  <div className="px-4 py-3 bg-muted/20 border-t border-border/50 animate-in slide-in-from-top-2">
                    <div className="flex gap-2 items-center">
                      ...
                      <input 
                        ...
                        className="bg-[#F0F2F5] dark:bg-[#3A3B3C] border-none rounded-full flex-1 px-4 py-2 text-sm focus:outline-none"
                      />
                    </div>
                  </div>
                )}
...
const StatusBtn = ({ icon, label }: any) => (
  <button className="flex items-center gap-2 p-2 hover:bg-muted rounded-lg transition-colors flex-1 justify-center font-semibold text-[14px] text-muted-foreground">
    {icon} {label}
  </button>
);

const ActionBtn = ({ icon, label }: any) => (
  <button className="flex-1 flex items-center justify-center gap-2 p-2 hover:bg-muted rounded-lg transition-colors font-semibold text-sm text-muted-foreground">
    {icon} {label}
  </button>
);
