  ...
  return (
    <DashboardLayout>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="mb-8">
          <h1 className="mb-2 font-display text-3xl font-bold text-foreground">Vendor Dashboard</h1>
          <p className="text-muted-foreground">Manage your temple listing, products, and orders.</p>
        </div>
              ...
              <CardContent className="p-0">
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4">
                  <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-lg bg-primary/10 border border-primary/20">
                    {temple.image_url ? (
                      <img src={temple.image_url} alt={temple.name} className="h-full w-full object-cover rounded-lg" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground">{temple.district}, {temple.province} • {application?.business_name}</p>
                  </div>
                  <div className="flex gap-2 w-full sm:w-auto">
                    <Link to="/vendor/temple" className="flex-1 sm:flex-none">
                      <Button variant="outline" size="sm" className="w-full gap-2"><Package className="h-4 w-4" />Manage</Button>
                    </Link>
                    <Link to={`/temples/${temple.id}`} className="flex-1 sm:flex-none">
                      <Button variant="ghost" size="sm" className="w-full gap-2"><ExternalLink className="h-4 w-4" />View</Button>
                    </Link>
                  </div>
                </div>
              </CardContent>
