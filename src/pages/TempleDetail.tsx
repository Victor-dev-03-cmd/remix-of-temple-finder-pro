...
const ExpandableReviewCard = ({ review, onUpdate }: { review: any, onUpdate: () => void }) => {
  ...
  return (
    <div className={`flex flex-col rounded-xl border bg-card p-5 shadow-sm transition-all duration-300 ${isExpanded ? 'ring-1 ring-primary/20' : ''}`}>
      ...
      <div className={`overflow-hidden transition-all duration-300 ${isExpanded ? 'max-h-[1000px]' : 'max-h-[60px]'}`}>
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-[45vh] min-h-[350px] overflow-hidden">
        ...
        <div className="container relative flex h-full flex-col justify-end pb-8">
          <Link to="/temples" className="mb-4 inline-flex"><Button variant="outline" size="sm" className="gap-2 bg-background/20 backdrop-blur-md border-white/20 text-white hover:bg-background/40"><ArrowLeft className="h-4 w-4" /> Back</Button></Link>
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div><h1 className="mb-2 font-display text-4xl font-semibold text-white sm:text-6xl drop-shadow-md">{temple.name}</h1><div className="flex items-center gap-2 text-white/90 font-medium"><MapPin className="h-4 w-4 text-primary" /> <span>{temple.district}, {temple.province}</span></div></div>
      ...
      <section className="py-12">
        <div className="container">
          {/* Main Grid: Desktop 2:1 ratio | Mobile: 1 column */}
          <div className="flex flex-col lg:grid lg:grid-cols-3 gap-10">
            
            {/* 1. ABOUT THE TEMPLE (Top in all views) */}
            <div className="lg:col-span-2 order-1 space-y-4">
              ...
              <div className="relative">
                <p className={`text-muted-foreground leading-relaxed text-lg ${!isAboutExpanded ? 'line-clamp-[10] mobile:line-clamp-[10]' : ''}`}>
                  {temple.description}
                </p>
            ...
            {/* 2. MAP & CONTACT INFO (Mobile: Order 2 | Desktop: Order 2 Sidebar) */}
            <div className="order-2 lg:order-none space-y-6">
              <div className="rounded-2xl border p-6 bg-card shadow-sm space-y-4">
                ...
                <div onClick={handleMapRedirect} className="aspect-video rounded-xl relative overflow-hidden cursor-pointer group border shadow-inner bg-slate-100">
                   ...
                   <div className="absolute inset-0 flex items-center justify-center group-hover:bg-primary/5">
                      <div className="bg-primary p-3 rounded-full shadow-2xl relative"><MapPin className="h-6 w-6 text-white" /></div>
                   </div>
                   <div className="absolute bottom-2 right-2 bg-white/80 backdrop-blur-sm px-2 py-1 rounded text-[10px] font-semibold text-primary">VIEW ON MAP</div>
                </div>
              ...
              <div className="rounded-2xl border p-6 bg-card shadow-sm space-y-5">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-4 p-3 rounded-xl bg-muted/40">
                    ...
                    <div><p className="text-xs text-muted-foreground font-semibold uppercase">Phone</p><p className="font-semibold">{temple.contact || 'Not available'}</p></div>
                  </div>
                  <div className="flex items-center gap-4 p-3 rounded-xl bg-muted/40">
                    <div className="h-10 w-10 bg-primary/10 rounded-lg flex items-center justify-center text-primary"><Clock size={18}/></div>
                    <div><p className="text-xs text-muted-foreground font-semibold uppercase">Opening Hours</p><p className="font-semibold">{temple.opening_hours || '6:00 AM - 8:00 PM'}</p></div>
                  </div>
                </div>
              </div>
            </div>

            {/* 3. WRITE A REVIEW FORM (Mobile: Order 3 | Desktop: Order 3 Sidebar) */}
            <div className="order-3 lg:order-none">
              <div className="rounded-2xl border bg-primary/5 p-6 border-primary/20 shadow-xl backdrop-blur-sm relative overflow-hidden">
                ...
                <h3 className="mb-4 font-semibold text-lg flex items-center gap-2 text-primary">
                  <Star className="fill-primary" size={20} /> Share Your Experience
                </h3>
                <TempleReviewForm key={formKey} templeId={id || ''} onSuccess={handleReviewSuccess} />
              </div>
            </div>

            {/* 4. DEVOTEE EXPERIENCES (Mobile: Order 4) */}
            <div className="lg:col-span-2 order-4 pt-8 border-t space-y-6">
              ...
              {reviewsLoading ? (
                ...
                <div className="text-center py-12 bg-muted/30 rounded-2xl border-2 border-dashed"><p className="text-muted-foreground font-medium">No experiences shared yet.</p></div>
              )}
            </div>

            {/* 5. GALLERY (Mobile: Order 5) */}
            <div className="lg:col-span-2 order-5 pt-8 space-y-4">
              <h3 className="font-display text-2xl font-semibold">Temple Gallery</h3>
              <div className="h-[300px] sm:h-[450px] flex gap-4">
                ...
                <div className="hidden sm:flex flex-col gap-3 overflow-y-auto no-scrollbar w-24">
                  {galleryImages.map((img, i) => (
                    <img key={i} src={img} onClick={() => setCurrentImageIndex(i)} className={`h-24 rounded-xl cursor-pointer object-cover border-2 ${i === currentImageIndex ? 'border-primary' : 'border-transparent opacity-50'}`} />
                  ))}
                </div>
              </div>
            </div>
      ...
      <section className="py-20 border-t bg-muted/20">
        <div className="container">
          ...
          {productsLoading ? (
            ...
            <div className="text-center py-16 bg-card rounded-2xl border-2 border-dashed"><Package className="h-12 w-12 text-muted-foreground/30 mx-auto mb-4" /><p className="text-muted-foreground">No products listed yet.</p></div>
          )}
        </div>
      </section>
